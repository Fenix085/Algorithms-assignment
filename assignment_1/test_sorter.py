...
class TestHeapSort(unittest.TestCase):
    def test_heap_sorted(self):
        arr = [16, 40, 52, 71, 86]
        sort_arr = [16, 40, 52, 71, 86]
        self.assertEqual(oSorter.heapSort(arr), sort_arr)
    def test_heap_reversed(self):
        arr = [86, 71, 52, 40, 16]
        sort_arr = [16, 40, 52, 71, 86]
        self.assertEqual(oSorter.heapSort(arr), sort_arr)
    def test_heap_empty(self):
        ...
        sort_arr = []
        self.assertEqual(oSorter.heapSort(arr), sort_arr)
    def test_heap_one_element(self):
        arr = [42]
        sort_arr = [42]
        self.assertEqual(oSorter.heapSort(arr), sort_arr)
    def test_heap_all_negative(self):
        arr = [-3, -1, -7, -4]
        sort_arr = [-7, -4, -3, -1]
        self.assertEqual(oSorter.heapSort(arr), sort_arr)
    def test_heap_random(self):
        arr = [3, -2, 5, 1, 0, -4]
        sort_arr = [-4, -2, 0, 1, 3, 5]
        self.assertEqual(oSorter.heapSort(arr), sort_arr)
